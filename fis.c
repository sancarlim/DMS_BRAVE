...
static DOUBLE fisTriangleMf(DOUBLE x, DOUBLE *params)
{
    ...
    return(MAX_FIS(MIN_FIS((x-a)/(b-a), (c-x)/(c-b)), 0));
}
...
static DOUBLE fisTrapezoidMf(DOUBLE x, DOUBLE *params)
{
    ...
    return(MIN_FIS(y1, y2));
	/*
	if (a == b && c == d)
		return((b<=x)*(x<=c));
	if (a == b)
        return(MIN_FIS(1, (d-x)/(d-c))*(b<=x)*(x<=d));
	if (c == d)
        return(MIN_FIS((x-a)/(b-a), 1)*(a<=x)*(x<=c));
    return(MAX_FIS(MIN_FIS(MIN_FIS((x-a)/(b-a), 1), (d-x)/(d-c)), 0));
	*/
}
...
void fisCheckDataStructure(FIS *fis)
{
	...
	for (i = 0; i < fis->rule_n; i++) {
		for (j = 0; j < fis->in_n; j++) {
            ...
            if (ABS_FIS(mf_index) > fis->input[j]->mf_n) {
		...
		for (j = 0; j < fis->out_n; j++) {
            ...
            if (ABS_FIS(mf_index) > fis->output[j]->mf_n) {
...
#endif
{
    ...
    col_n = MAX_FIS(col_n, 8);	/* 'centroid' defuzzification */
	/* find FIS type */
	while (1) {
		if (getNextLine(buf, fp) == NULL)
	...
	while (getNextLine(buf, fp) != NULL) {
 		if (sscanf(buf, " Name = '%[^']' ", IoName) == 1)
            col_n = MAX_FIS(col_n, (int)strlen(IoName));
		if (sscanf(buf, " %[^'] '%[^']' : '%[^']' , [ %[^]] ",
			tmp1, tmp2, tmp3, tmp4) == 4) {
            col_n = MAX_FIS(col_n, (int)strlen(tmp2));
            col_n = MAX_FIS(col_n, (int)strlen(tmp3));
		}
	}
	if (!strcmp(fisType, "mamdani")) {
          col_n = MAX_FIS(col_n, MF_PARA_N);
          /* When number of inputs/outputs larger than length of strings. */
          col_n = MAX_FIS(col_n, in_n+out_n+2);
