...
void Visualizer::SetDIP(double DIP, double DLI)
{
	char DIPC[100];
	char DLIC[100];
	std::sprintf(DIPC, "%.2f", (float)DIP);
	std::string DIPStr("DIP: ");
	std::sprintf(DLIC, "%.2f", (float)DLI);
	std::string DLIStr("DLI: ");
	DIPStr += DIPC;
	DIPStr += ", ";
	DLIStr += DLIC;
	DLIStr += ", ";

	cv::putText(captured_image, DIPStr,cvPoint(10,50),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(255,0,0),1.5);
	cv::putText(captured_image, DLIStr,cvPoint(150,50),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(255,0,0),1.5);
	//if (DIP >= 0.5)
		//cv::putText(captured_image, "WARNING ALARM",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
}


void Visualizer::SetIL(double IL)
{
	char IL_C[100];
	std::sprintf(IL_C, "%.2f", (float)IL);
	std::string ILStr("Inattention Level: ");
	ILStr += IL_C;
	cv::putText(captured_image, ILStr,cvPoint(270,50),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(255,0,0),1.5);
	if (IL < 0.2)
		cv::putText(captured_image, "Inattention Level: LOW",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
	else if (0.2 < IL && IL <= 0.55)
		cv::putText(captured_image, "Inattention Level: MEDIUM-LOW",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
	else if (0.55 < IL && IL < 0.8)
		cv::putText(captured_image, "Inattention Level: MEDIUM-HIGH",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
	else if (IL > 0.8)
		cv::putText(captured_image, "Inattention Level: HIGH",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
}

void Visualizer::SetParameters(double Aperture,int yawn, double yawning, double blink, double PERCLOS, int nblinks)
{
	char apertureC[100];
	char yawnC[100];
	char blinkC[100];
	...
	std::sprintf(n_blinksC, "%d", nblinks);
	if (count % 3 == 0)
	{
	std::sprintf(apertureC, "%.1f ", (float)Aperture);
	std::sprintf(yawnC, ", yawning: %.2f ", yawning);
	std::sprintf(blinkC, ", blink_rate:%.2f ", (float)blink);
	std::sprintf(PERCLOSC, ", PERCLOS:%.2f ", (float)PERCLOS);
	}
	std::string params("Aperture: ");
	params += apertureC;
	params +=yawnC;
	params +=blinkC;
	params +=PERCLOSC;
	std::string blinkstr("n blinks: "); 
	blinkstr += n_blinksC;
	cv::putText(captured_image, blinkstr,cvPoint(10,70),cv::FONT_HERSHEY_SIMPLEX,0.5,CV_RGB(255,0,0),1);
	/*if (nblinks == 1)
		cv::putText(captured_image, "BLINK",cvPoint(10,70),cv::FONT_HERSHEY_SIMPLEX,0.5,CV_RGB(255,0,0),1);*/

	//if (yawn==1)
	//	cv::putText(captured_image, "YAWN", cvPoint(10,70),cv::FONT_HERSHEY_SIMPLEX,0.5,CV_RGB(255,0,0),1);

	cv::putText(captured_image, params,cvPoint(10,90),cv::FONT_HERSHEY_SIMPLEX,0.5,CV_RGB(255,0,0),1);
}


void Visualizer::SetGaze(cv::Vec2d gaze, int count, int state)
{
	char GazeC[255];
	cv::Vec2d gaze_vis;
	...
	std::sprintf(GazeC, "%d, %d", int(gaze_vis[0]*100), int(gaze_vis[1]*100));

	std::string GazeStr("Gazex, gazey: ");
	GazeStr += GazeC;

	if (state == 3) 		
		cv::putText(captured_image, "Looking at driving mirror",cvPoint(10,550),cv::FONT_HERSHEY_SIMPLEX,0.8,CV_RGB(0,255,0),1.9);
	else if (state == 1)
		{
		cv::putText(captured_image, "Using mobile phone !",cvPoint(10,550),cv::FONT_HERSHEY_SIMPLEX,0.8,CV_RGB(0,255,0),1.9);
		//cv::putText(captured_image, "WARNING ALARM",cvPoint(10,590),cv::FONT_HERSHEY_SIMPLEX,0.7,CV_RGB(0,0,255),1.9);
		}	
	else if (state == 2)
		cv::putText(captured_image, "Looking at on-board screen",cvPoint(10,550),cv::FONT_HERSHEY_SIMPLEX,0.8,CV_RGB(0,255,0),1.9);
	else 
		//if (count_state == 1)
			cv::putText(captured_image, "Looking at road scene",cvPoint(10,550),cv::FONT_HERSHEY_SIMPLEX,0.8,CV_RGB(0,255,0),1.9);


	
	cv::putText(captured_image, GazeStr,cvPoint(10,110),cv::FONT_HERSHEY_SIMPLEX,0.5,CV_RGB(255,0,0),1.8);
}
